	...
	class DatabaseUtil {
		...
		public static function beginTransaction($con){
			...
			if($_SESSION[DATABASE_TRANSACTION_COUNTER] == 1 && $_SESSION[DATABASE_TRANSACTION_LEAK_TIMESTAMP] == 0){
				//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_ERROR, "Suspected database transaction leak !");
			...
			//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_INFO, "Database Transaction Status : started [transactions alive-	".$_SESSION[DATABASE_TRANSACTION_COUNTER]."]");
			//audit
		}
		...
		public static function endTransaction($con){
			...
			if($_SESSION[DATABASE_TRANSACTION_COUNTER] == 0){
				//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_ERROR, "Expecting atleast 1 database transaction to be alive here but found (".$_SESSION[DATABASE_TRANSACTION_COUNTER]."). Premature connection closed ?");
			}	
			
			$_SESSION[DATABASE_TRANSACTION_COUNTER] = $_SESSION[DATABASE_TRANSACTION_COUNTER] - 1;
			//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_INFO, "Database Transaction Status : ended [transactions alive-".$_SESSION[DATABASE_TRANSACTION_COUNTER]."]");
			//audit
		}
		
		public static function rollbackTransaction($con){
			mysqli_rollback($con);
			//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_INFO, "Database Transaction Status : rolled back");
		}
		...
		public static function open_connection() {
			...
			if(! $conn) {
				//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_ERROR, "Failed to connect to MySQL: " . mysqli_connect_error());
				...
				//	Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_ERROR, "Suspected database connection leak !");
			...
			//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_INFO, "Database Status : connected [connections alive-".$_SESSION[DATABASE_CONNECTION_COUNTER]."]");
		...
		public static function close_connection($conn) {
			...
			if($_SESSION[DATABASE_CONNECTION_COUNTER] == 0){
				//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_ERROR, "Expecting atleast 1 database connection to be alive here but found (".$_SESSION[DATABASE_CONNECTION_COUNTER]."). Premature connection closed ?");
			}	
			
			$_SESSION[DATABASE_CONNECTION_COUNTER] = $_SESSION[DATABASE_CONNECTION_COUNTER] - 1;
			//Utility::logger(__CLASS__, __METHOD__, __LINE__, LOG_TYPE_INFO, "Database Status : disconnected [connections alive-".$_SESSION[DATABASE_CONNECTION_COUNTER]."]");
			//audit
		}
