<?php
	...
	class Recipe{
		...
		public static function fetchSubmitRecipe($rcp_id, $rcp_nm, $food_csn_nm, $ing_id, $ing_nm, $qty_id, $ing_qty, $rcp_proc, $rcp_steps, $rcp_plating, $rcp_note,
			...
			//request
			logger(__CLASS__, "I", "REQUEST PARAM : rcp_nm(".$rcp_nm.")");
			logger(__CLASS__, "I", "REQUEST PARAM : food_csn_nm(".$food_csn_nm.")");
			logger(__CLASS__, "I", "REQUEST PARAM : rcp_proc(".$rcp_proc.")");
			logger(__CLASS__, "I", "REQUEST PARAM : rcp_plating(".$rcp_plating.")");
			logger(__CLASS__, "I", "REQUEST PARAM : rcp_note(".$rcp_note.")");
			logger(__CLASS__, "I", "REQUEST PARAM : food_typ_id(".$food_typ_id.")");
			...
			for ($i = 0; $i < count($rcp_steps); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : rcp_steps[".$i."](".$rcp_steps[$i].")");
			}

			for ($i = 0; $i < count($ing_id); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : ing_id[".$i."](".$ing_id[$i].")");
			}

			for ($i = 0; $i < count($ing_nm); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : ing_nm[".$i."](".$ing_nm[$i].")");
			}

			for ($i = 0; $i < count($qty_id); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : qty_id[".$i."](".$qty_id[$i].")");
			}
			...
			for ($i = 0; $i < count($tst_id); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : tst_id[".$i."](".$tst_id[$i].")");
			}

			for ($i = 0; $i < count($tst_qty); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : tst_qty[".$i."](".$tst_qty[$i].")");
			}

			for ($i = 0; $i < count($rcp_images['tmp_name']); $i++) {
				logger(__CLASS__, "I", "REQUEST PARAM : rcp_images[".$i."](".$rcp_images['tmp_name'][$i].")");
			} 
			//arrays
			//request
			...
			if(!check_for_null($user_id)){
				logger(__CLASS__, "E", "Error ! null/empty user_id)");
				return;
			}

			if(!check_for_null($rcp_nm)){
				logger(__CLASS__, "E", "Error ! null/empty rcp_nm");
				return;
			}

			if(!check_for_null($food_typ_id)){
				logger(__CLASS__, "E", "Error ! null/empty food_typ_id");
				return;
			}

			if(!check_for_null($food_csn_nm)){
				logger(__CLASS__, "E", "Error ! null/empty food_csn_nm");
				return;
			}

			if(count($ing_id) == 0){
				logger(__CLASS__, "E", "Error ! null/empty ing_id");
				return;
			}

			if(!check_for_null($qty_id)){
				logger(__CLASS__, "E", "Error ! null/empty qty_id");
				return;
			}

			if(!check_for_null($ing_qty)){
				logger(__CLASS__, "E", "Error ! null/empty ing_qty");
				return;
			}

			if(!check_for_null($rcp_proc)){
				logger(__CLASS__, "E", "Error ! null/empty rcp_proc");
				return;
			}

			if(!check_for_null($tst_id)){
				logger(__CLASS__, "E", "Error ! null/empty tst_id");
				return;
			}

			if(!check_for_null($tst_qty)){
				logger(__CLASS__, "E", "Error ! null/empty tst_qty");
				return;
			}  

			if(count($rcp_images['name']) == 0){
				logger(__CLASS__, "E", "Error ! null/empty rcp_images");
				return;
			}

			if(count($rcp_steps) == 0){
				logger(__CLASS__, "E", "Error ! null/empty rcp_steps");
				return;
			}
			//mandatory
			...
			if(!check_for_null($rcp_id)){
				logger(__CLASS__, "I", "null/empty rcp_id");
			}

			if(!check_for_null($ing_nm)){
				logger(__CLASS__, "I", "null/empty ing_nm");
			}

			if(!check_for_null($rcp_plating)){
				logger(__CLASS__, "I", "null/empty rcp_plating");
			}

			if(!check_for_null($rcp_note)){
				logger(__CLASS__, "I", "null/empty rcp_note");
			}
			//optional
			//check for null/empty
			...
			try{
				...
				if(check_for_null($rcp_id)){
					logger(__CLASS__, "I", "User(".$user_id.") is updating an old Recipe(".$rcp_id.")");
					...
					if(mysqli_query($con, $query)){
						logger(__CLASS__, "I", "Recipe(".$rcp_id.") updated into RECIPE table by User(".$user_id.")");

						//delete the old ingredients in DISH table
						$query = "DELETE FROM `DISH` WHERE RCP_ID = '".$rcp_id."'";
						if(mysqli_query($con, $query)){
							logger(__CLASS__, "I", "All Ingredients for Recipe(".$rcp_id.") are deleted from DISH table");
						}
						else{
							logger(__CLASS__, "E", "Error ! Failed to delete Ingredients for Recipe(".$rcp_id.") from DISH table.");
							logger(__CLASS__, "E", "Failed query : ".$query);
							logger(__CLASS__, "E", "Rolling back !");
						...
						for($i = 0; $i< count($ing_id); $i++){
							...
							if($temp_ing_id == null || $temp_ing_id == ''){
								...
								if(mysqli_query($con, $query)){
									...
									logger(__CLASS__, "I", "Ingredient(".$temp_ing_id.") added into INGREDIENT table");
								}
								else{
									logger(__CLASS__, "E", "Error ! Failed to insert into INGREDIENT table.");
									logger(__CLASS__, "E", "Failed query : ".$query);
									logger(__CLASS__, "E", "Rolling back !");
							...
							if(mysqli_query($con, $query)){
								...
								logger(__CLASS__, "I", "Ingredient(".$dish_id.") added into DISH table");
							}
							else{
								logger(__CLASS__, "E", "Error ! Failed to insert into DISH table.");
								logger(__CLASS__, "E", "Failed query : ".$query);
								logger(__CLASS__, "E", "Rolling back !");
						...
						if(mysqli_query($con, $query)){
							logger(__CLASS__, "I", "All steps for Recipe(".$rcp_id.") are deleted from RECIPE_STEPS table");
						}
						else{
							logger(__CLASS__, "E", "Error ! Failed to delete steps for Recipe(".$rcp_id.") from RECIPE_STEPS table.");
							logger(__CLASS__, "E", "Failed query : ".$query);
							logger(__CLASS__, "E", "Rolling back !");
						...
						for($i = 0; $i< count($rcp_steps); $i++){
							...
							if(mysqli_query($con, $query)){
								...
								logger(__CLASS__, "I", "Recipe Step(".$rcp_stps_id.") added into RECIPE_STEPS table");
							}
							else{
								logger(__CLASS__, "E", "Error ! Failed to insert into RECIPE_STEPS table.");
								logger(__CLASS__, "E", "Failed query : ".$query);
								logger(__CLASS__, "E", "Rolling back !");
						...
						if(mysqli_query($con, $query)){
							logger(__CLASS__, "I", "All tastes for Recipe(".$rcp_id.") are deleted from RECIPE_TASTE table");
						}
						else{
							logger(__CLASS__, "E", "Error ! Failed to delete tastes for Recipe(".$rcp_id.") from RECIPE_TASTE table.");
							logger(__CLASS__, "E", "Failed query : ".$query);
							logger(__CLASS__, "E", "Rolling back !");
						...
						for($i = 0; $i< count($tst_id); $i++){
							...
							if(mysqli_query($con, $query)){
								...
								logger(__CLASS__, "I", "Taste(".$rcp_tst_id.") added into DISH table");
							}
							else{
								logger(__CLASS__, "E", "Error ! Failed to insert into RECIPE_TASTE table.");
								logger(__CLASS__, "E", "Failed query : ".$query);
								logger(__CLASS__, "E", "Rolling back !");
					...
					if(mysqli_query($con, $query)){
						logger(__CLASS__, "I", "All images for Recipe(".$rcp_id.") are deleted from RECIPE_IMG table");
					...
					if(!prepare_directories($user_id)){
						logger(__CLASS__, "E", "Cannot submit the recipe as the file directories could not be created for the user($user_id).");  
						logger(__CLASS__, "E", "Rolling back !");
					...
					if(!create_directory($recipe_images_dir)){
						logger(__CLASS__, "E", "Error ! Failed to create directory(".$recipe_images_dir.")");
						logger(__CLASS__, "E", "Rolling back !");
					...
					for($i = 0; $i< count($rcp_images['tmp_name']); $i++){
						try{
							...
							if(mysqli_query($con, $query)){
								...
								logger(__CLASS__, "I", "Recipe Image(".$rcp_img_id.") added into RECIPE_IMG table");
							}
							else{
								logger(__CLASS__, "E", "Error ! Failed to insert into RECIPE_IMG table.");
								logger(__CLASS__, "E", "Failed query : ".$query);
								logger(__CLASS__, "E", "Rolling back !");
							...
							if (isset($rcp_images['tmp_name'][$i])){
								...
								logger(__CLASS__, "I", "Recipe Image(".$recipe_image.") uploaded");
							}
						}
						catch(Exception $e){
							logger(__CLASS__, "E", "Image upload fail exception : ".$e->getMessage());
							$upload_failed = true;
						}
					}

					if(!$atleast_one_uploaded){
						logger(__CLASS__, "I", "Could not upload atleast one recipe image");
						logger(__CLASS__, "E", "Rolling back !");
				...
				//if the $rcp_id is null/empty, user is adding a new recipe
					...
					logger(__CLASS__, "I", "User(".$user_id.") is adding a new Recipe");
					...
					if(mysqli_query($con, $query)){
						...
						logger(__CLASS__, "I", "Recipe(".$rcp_id.") added into RECIPE table by User(".$user_id.")");
		...
		public static function fetchUserViewedRecipes($user_id){
			...
			try{
				...
				logger(__CLASS__, "I", "Total Recipes fetched : ".sizeof($result_array));
		...
		public static function fetchRecipe($rcp_id, $user_id){
			...
			//validations
			if(!check_for_null($rcp_id)){
				logger(__CLASS__, "E", "Error ! null/empty recipe id");
				return;
			}
			...
			try{
				...
				logger(__CLASS__, "E", 'Error ! Could not fetch recipe');
				logger(__CLASS__, "E", $e->getMessage());
			}
			finally{
				DatabaseUtil::getInstance()->close_connection($con);
			}
		...
		public static function deleteRecipe($rcp_id, $user_id){
			...
			try{
				...
				if(mysqli_query($con, $query)){
					logger(__CLASS__, "I" , "Recipe('$rcp_id') successfully deleted by the user('$user_id')");
					echo "SUCCESS";
					...
					logger(__CLASS__, "E", "Failed !! Recipe('$rcp_id') could not be deleted by the user('$user_id')");
					echo "FAIL";
				} 
				//delete recipe
			}
