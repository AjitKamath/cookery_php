<?php
	class Timeline{
		...
		public static function fetchUserFollowsTimeline($user_id, $index){
			//request
			LoggerUtil::logger(__CLASS__, __METHOD__, __LINE__, "I", "REQUEST PARAM : user_id(".$user_id.")");
			LoggerUtil::logger(__CLASS__, __METHOD__, __LINE__, "I", "REQUEST PARAM : index(".$index.")");
			//request
			...
			try{
				...
				$query = "SELECT DISTINCT TMLN_ID, TYPE, TYPE_ID, TMLN.CREATE_DTM, USR.USER_ID, USR.NAME, USR.IMG 
							FROM `TIMELINES` AS TMLN
							INNER JOIN `USER` AS USR ON USR.USER_ID = TMLN.USER_ID
							INNER JOIN `USER_RELATIONSHIP` AS USR_RLT ON USR_RLT.FLWR_USER_ID = TMLN.USER_ID
							WHERE TMLN.USER_ID = '$user_id'
							AND TMLN.TYPE IN ('".RECIPE_ADD."', '".RECIPE_MODIFY."', '".RECIPE_REMOVE."', '".USER_PHOTO_MODIFY."')
							ORDER BY TMLN.CREATE_DTM DESC
							LIMIT $index , ".TIMELINES_COUNT;
				$result = mysqli_query($con, $query);
				...
				while($result_data = $result->fetch_object()){
					...
					if(RECIPE_ADD == $type || RECIPE_MODIFY == $type || RECIPE_REMOVE == $type){
						$timeline_query = "SELECT RCP.RCP_ID, RCP_NAME, FOOD_TYP_NAME, FOOD_CSN_NAME,
										USR.NAME AS WHOSE_NAME, USR.USER_ID AS WHOSE_USER_ID, USR.IMG AS RCP_OWN_IMG
										FROM `RECIPE` AS RCP
										INNER JOIN `FOOD_TYPE` AS FOOD_TYPE ON FOOD_TYPE.FOOD_TYP_ID = RCP.FOOD_TYP_ID
					...
					else if(USER_PHOTO_MODIFY == $type){
						$timeline_query = "SELECT CREATE_DTM, MOD_DTM FROM `USER` WHERE USER_ID = '$result_data->TYPE_ID'";
						$timeline_result = mysqli_query($con, $timeline_query);
